...
def cargar_materias(tree):
    ...
    filas = obtener_todos("""
        ...
        ORDER BY g.orden, e.nombre, m.orden
    """)

    grados = {}
    especialidades = {}

    for materia_id, grado, clave, nombre, orden, especialidad in filas:
...
def ventana_materia(parent, tree, datos=None):
    ...
    # ORDEN
    # =========================
    tk.Label(cont, text="orden:").pack(anchor="w")
    entry_orden = tk.Entry(cont, textvariable=orden_var)
    entry_orden.pack(fill="x", pady=5)
    ...
    def guardar():
        ...
        if not clave or not nombre or not grado_nombre or not orden:
            show_error(top, "Error", "Clave, nombre, grado y orden son obligatorios.")
            return
        ...
        if datos:  # MODIFICAR
            ...
            ejecutar_consulta("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (clave, nombre, grado_id, especialidad_id, orden))
